...
def downsample_doppler(input_array, downsample_factor, center_of_bright_line=64):
    ...
    for each_frame in data:
        output_array = np.expand_dims(each_frame[center_of_bright_line,:], axis=0)

        for i in range(center_of_bright_line+downsample_factor, data.shape[1], downsample_factor):
            to_concat = np.expand_dims(each_frame[i,:], axis=0)
            output_array = np.concatenate((output_array, to_concat), axis=0)
        
        for i in range(center_of_bright_line-downsample_factor, -1, downsample_factor*-1):
            to_concat = np.expand_dims(each_frame[i,:], axis=0)
            output_array = np.insert(output_array, [0], to_concat, axis=0)
